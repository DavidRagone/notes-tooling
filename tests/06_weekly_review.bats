...
- Ship feature flag
MD
  run_private .scripts/weekly-review --start 2025-11-03 --end 2025-11-09 --no-llm --no-open
  [ "$status" -eq 0 ]
...
@test "weekly-review with LLM stub includes a Summary section" {
  # seed files so LLM has content to process
  j="$PRIVATE/journal/2025/2025-11/2025-11-03.md"
  m="$PRIVATE/meetings/2025/2025-11/2025-11-04-sync.md"
  mkdir -p "$(dirname "$j")" "$(dirname "$m")"
  cat > "$j" <<'MD'
...
- Ship feature flag
MD
  run_private .scripts/weekly-review --start 2025-11-03 --end 2025-11-09 --no-open
  [ "$status" -eq 0 ]
  out="$PRIVATE/reviews/2025/2025-45.md"
  grep -q "^## Summary" "$out"
